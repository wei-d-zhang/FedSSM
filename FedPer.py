...
class Client(object):
    ...
    def load_global_model(self, global_model_state_dict):
        ...
        for name, param in global_model_state_dict.items():
            if 'fc' in name:
                local_param[name] = self.net.state_dict()[name]  # 保留原始参数
            else:
                local_param[name] = param  # 更新为全局参数
            
        self.net.load_state_dict(local_param)
    ...
    def train(self):
        for comm_round in range(self.args.comm_round):
            ...
            for client in self.clients:
                client.load_global_model(self.global_model.state_dict())
...
class Args:
    def __init__(self):
        ...
        self.method = 'FedPer'  #FedAvg/Ditto/Our/FedALA/Local/FedPer
